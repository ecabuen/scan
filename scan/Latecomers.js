...
export default function Late() {
  ...
  const fetchStudents = async () => {
    try {
      ...
      // Filter students who are late
      const lateStudents = response.data.data.filter(student => student.attendanceStatus === 'Late');

      if (lateStudents.length > 0) {
        setStudents(lateStudents); // If there are late students, display them
      } else {
        // If no late students, set all students with a 'No Record' status
    ...
    <View style={styles.container}>
      ...
      <View style={styles.headerContainer}>
        ...
        <View style={styles.header}>
          <Text style={styles.headerText}>Latecomers</Text>
        </View>
      </View>
