...
export default function Present() {
  ...
  const fetchStudents = async () => {
    try {
      ...
      const presentStudents = response.data.data.filter(student => student.attendanceStatus === 'Present');

      if (presentStudents.length > 0) {
        setStudents(presentStudents); // If there are present students, display them
      } else {
        // If no present students, set all students with a 'No Record' status
    ...
    <View style={styles.container}>
      ...
      <View style={styles.headerContainer}>
        ...
        <View style={styles.header}>
          <Text style={styles.headerText}>Present Students</Text>
        </View>
      </View>
